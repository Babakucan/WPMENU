...
function isOpen(rest) {
  ...
  if (Array.isArray(rest.specialDays)) {
    const ov = rest.specialDays.find((d) => String(d.date || '') === todayStr);
    if (ov) {
      if (ov.closed) return false;
      if (ov.open && ov.close) {
        return isTimeRangeOpen(now, ov.open, ov.close);
      }
    }
  }

  // 2) Weekly hours
  if (rest.weeklyHours && typeof rest.weeklyHours === 'object') {
    const dow = String(now.getDay());
    const dayCfg = rest.weeklyHours[dow];
    if (dayCfg) {
      if (dayCfg.enabled === false) return false;
      if (dayCfg.open && dayCfg.close) return isTimeRangeOpen(now, dayCfg.open, dayCfg.close);
    }
  }

  // 3) Legacy fallback
  if (!rest.hoursOpen || !rest.hoursClose) return true;
  return isTimeRangeOpen(now, rest.hoursOpen, rest.hoursClose);
}

function isTimeRangeOpen(nowDate, openStr, closeStr) {
...
function formatOrderConfirm(order, getRestaurantFn) {
  ...
  const payLabels = { online: 'Online ödeme', kapida_pos: 'Kapıda kart', kapida_nakit: 'Kapıda nakit' };
  const payLabel = payLabels[order.paymentMethod] || payLabels.kapida_nakit;
